...
class MultiModalTS(Dataset):
    ...
    1.  `x_blocks`: A dictionary of sequential features for TFT/TCN/TST.
    2.  `x_tabular`: A tensor of tabular features for XGBoost.
    3.  `y_dict`: A dictionary of multi-task targets (price, vol).

    IMPORTANT:
      - Preprocessing (returns & rolling stats) is now done via the shared
        apply_price_feature_config() so that training == inference.
    """
    def __init__(
        self,
        df: pd.DataFrame,
        ...
        # 0. Basic NaN/inf cleanup
        if dropna:
            df = df.replace([np.inf, -np.inf], np.nan).dropna()

        # 1. Apply SHARED preprocessing (same as inference)
        df = apply_price_feature_config(df)

        # 2. Create Tabular Features for XGBoost
        print("Creating tabular features for dataset...")
        tabular_df = create_tabular_features(df, tabular_feature_cols, roll_windows)

        # 3. Align all data (drop initial rows where tabular features are NaN)
        first_valid_idx = tabular_df.first_valid_index()
        if first_valid_idx is None:
            raise ValueError("Feature engineering resulted in an all-NaN tabular DataFrame.")
        ...
        # 4. Build Sequential Feature Matrix (X) for TFT/TCN/TST
        X_blocks = []
        ...
        # 5. Build Multi-Task Labels (aligned)
