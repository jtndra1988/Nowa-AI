...
# TCN channels (e.g., 3 levels deep)
...
def train_one_epoch(model, loader, optimizer, device, price_loss_params, vol_weight):
    ...
    for x_blocks, y_dict in loader:
        # x_blocks is a dict, move each tensor to device
...
def validate(model, loader, device, price_loss_params, vol_weight):
    ...
    # Note: No feature_weights to log for TCN
    return avg_total_loss, avg_price_loss, avg_vol_loss
...
def run_training():
    ...
    # Save the scaler
    scaler_path = ARTIFACT_DIR / "tcn_scaler.npz"
    ...
    # --- 5. Initialize Model ---
    # --- KEY CHANGE for TCN ---
    # Get total number of features from dataset
    in_feat = train_ds.X.shape[1]
    
    print(f"Initializing TCN model with {in_feat} total input features.")
    ...
    early_stopper = EarlyStopping(
        patience=10, 
        checkpoint_path=ARTIFACT_DIR / "tcn_best_model.pth"
    )
    ...
    print(f"Best TCN model saved to {early_stopper.checkpoint_path}")
